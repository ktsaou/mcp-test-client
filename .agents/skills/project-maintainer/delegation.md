...
goes to a Worker. If you find yourself editing files outside `.agents/`
and `AGENTS.md`, you are doing instead of judging.
...
Default rule: any file outside `.agents/` and `AGENTS.md` is Worker
territory. Maintainer-mode edits to source code require a written
justification (recorded in the active SOW or commit message).
...
- **2026-04-25 — Delegation discipline failure.** In a single session I
  ...
  of delegating." **Guardrail:** any file outside `.agents/` and
  `AGENTS.md` is now Worker territory by default; maintainer-mode edits
  to source code require a written justification (recorded in the active
  SOW or commit message).
