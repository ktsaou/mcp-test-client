...
| Situation                                    | Agent                                                                                                         |
| -------------------------------------------- | ------------------------------------------------------------------------------------------------------------- |
| Decision touches a user-visible surface      | UX critic (mandatory before merge — see [DEC-002](../../sow/specs/decisions/DEC-002-ux-advisor-mandatory.md)) |
...
Read first: AGENTS.md, .agents/sow/specs/values.md, .agents/sow/specs/, the
...
Full UX-critic recipe: `.agents/skills/project-reviewing/ux-review.md`. The
roster entry below is a brief summary.

The UX critic walks through the canonical 60-second flow as a first-time
user, grades against the quality bar, reports the top painful issues plus
the things that already feel good, and renders a ship / no-ship verdict.
The recipe in `.agents/skills/project-reviewing/ux-review.md` carries the
load-bearing details: mandatory test scopes (mobile breakpoints, share-link
reload, bearer-token auth, 200+ message log perf, modal a11y including
Enter-submit, alignment-under-squeeze with bucketed half-pixel measurement),
mandatory evidence forms (`_evaluate` JSON, `_snapshot` YAML, screenshots
optional), deliverable shape, and the falsifier.
...
60-second flow in .agents/sow/specs/60-second-flow.md, and you didn't
catch it.
```
...
Read first: AGENTS.md, the relevant specs/ file(s), the relevant
.agents/sow/specs/decisions/DEC-NNN-*.md entry (if any).
